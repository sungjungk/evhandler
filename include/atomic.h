...
typedef atomic_t atomic_lock_t;

#define atomic_lock_init(l)    atomic_store(l, 0)
...
static inline void
atomic_lock(volatile atomic_lock_t *lk)
{
    while (atomic_cas(lk, 1, 0));
	asm __volatile__("mfence" ::: "memory");
}

static inline void
atomic_unlock(volatile atomic_lock_t *lk)
{
	asm __volatile__("mfence" ::: "memory");
    atomic_store(lk, 0);
}
